...
class Sphere {
    ...
    gravityVelocityVector: THREE.Vector3;
    inertiaVelocity: number;  //orbital velocity by default 
    inertiaVelocityVector: THREE.Vector3;
    velocityVector: THREE.Vector3; //final velocity when added gravity and orbital
    ...
    constructor(r: number, x: number, y: number, z: number, mass: number, texture: SphereTexture, id?: string) {
    ...
    distance(otherSphere: Sphere): number {
    ...
    directionVectorToOther(otherSphere: Sphere): THREE.Vector3 {
    ...
    directionVectorToThis(otherSphere: Sphere): THREE.Vector3 {
    ...
    calculate_g(objects: Sphere[], delta: number): THREE.Vector3 {
    ...
    setOrbitalVelocity(object: Sphere) {
        if (object == this) {
            this.inertiaVelocity = 0;
        } else {
    ...
    collision(objects: Sphere[]): boolean {
        if (params.allowCollisions) {
            let colides: boolean = false;
            for (let i = 0; i < objects.length; i++) {
                if (objects[i] != this) {
...
function createPlanets(sizeMulti: number) {
...
function turnVectorRight(vector: THREE.Vector3) {
    var axis = new THREE.Vector3(0, -1, 0);
    var angle = 90 * (Math.PI / 180);
...
function turnVectorLeft(vector: THREE.Vector3) {
    var axis = new THREE.Vector3(0, 1, 0);
    var angle = 90 * (Math.PI / 180);
    let newVec = vector.clone();
    newVec.applyAxisAngle(axis, angle);
